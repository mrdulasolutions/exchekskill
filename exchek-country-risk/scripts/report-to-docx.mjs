#!/usr/bin/env node
/**
 * Convert an ExChek country risk one-pager markdown (.md) to Word (.docx).
 * Usage: node report-to-docx.mjs <path-to-one-pager.md>
 * Output: <path-to-one-pager.docx> in the same directory.
...
async function main() {
  ...
  if (!mdPath) {
    console.error("Usage: node report-to-docx.mjs <path-to-one-pager.md>");
    process.exit(1);
  }
